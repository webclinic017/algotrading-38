...
def index(request: Request):
    ...
    latest_day=(date.today()-timedelta(days=1)).isoformat()
    #edit this line according to when you are running cron job
    
    cursor.execute("""
        SELECT symbol,rsi_14,sma_20,sma_50, close
        FROM stock JOIN stock_price on stock_id=stock.id
        WHERE date = ?;
    """, (latest_day,))
